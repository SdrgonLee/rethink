...
import { createServerTlsOptions } from './util/sni-certificates'
...
const tlsServerOptions = createServerTlsOptions(ca, config.sni_certificates)
...
function t1setup(manager: DeviceManager) {
    ...
    https.createServer(tlsServerOptions, app).listen(config.thinq1_https_port.bind, config.thinq1_https_port.address)
    const acceptor = new T1Acceptor()
    tls.createServer(tlsServerOptions, acceptor.accept.bind(acceptor)).listen(
...
function t2setup(manager: DeviceManager) {
    ...
    https.createServer(tlsServerOptions, app).listen(config.https_port.bind, config.https_port.address)

    // internal MQTT broker
    const broker = new Broker()

    if (config.mqtt) {
        tls.createServer(tlsServerOptions, broker.accept.bind(broker)).listen(
